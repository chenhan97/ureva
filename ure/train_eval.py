...
def train(model, dataset, config):
    ...
    for e in range(30):
        ...
        while True:
            ...
            if count % 200 == 0:
                s, v_m, ari = evaluate(model, dataset=dataset, batchsize=config["batchsize"], data=dataset.dev)
                if s>dev_best:
                    dev_best = s
                    print("best b3:", s)
                else:
                    print("best b3:", dev_best)
            start = end

        scheduler.step()
        print('-'*30, 'dev')
        s,_,_ = evaluate(model, dataset=dataset, batchsize=config["batchsize"], data=dataset.dev)
        if s > dev_best:
            dev_best = s
            not_inc_count = 0
...
def evaluate(model, dataset, batchsize=100, data=None, printing=True):
    ...
    while True:
        ...
        _input = dataset.get_minibatch(data, start, end)
        predictions,_,[_,_] = model.predict_relation(_input)
